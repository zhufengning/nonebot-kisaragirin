...
17. [x] 支持 route fan-out：一次触发同时走多条路径
   - 允许同一批新消息在 route 阶段拆成多个并行分支，而不是只能选择单一路径。
   - 典型场景：同一轮消息里同时存在“吹牛上下文”和“技术提问”，分别走不同中段图。
   - 当前进展：`route` 节点已改为输出路径数组；每条路径会带自己的处理要求执行中段图，并允许单路径沉默。

18. [x] 引入 emission / output-event 机制，替代“单次 return”
   - 不让节点直接把值 return 给外层，而是在图状态中写入 `emission` / `output_event`。
   - 外层执行器监听 emission，按完成顺序流式发送多条消息，而不是等所有路径跑完后一次性堆到最后。
   - 当前进展：已定义 `OutputEvent`，包含消息内容、来源路径、final、排序键与去重键；插件侧按顺序消费输出事件。

19. [x] 明确多路径多消息下的发送、记忆与失败语义
   - 约束哪些节点可以 emit，一般只允许 reply 类节点 emit，避免任意节点抢着对外发消息。
   - 明确是否允许单路径多次 emit、不同路径是否都可 emit，以及发送顺序按完成时间还是按优先级。
   - 当前进展：只有 reply 类节点会产出输出事件；整轮记忆会在全部路径结束后统一写回，并且只记录成功发送的路径回复。若部分路径已发送成功后失败，为避免重复发送，不回灌整轮快照。
## P3：最后补工程化护栏
