...
- `reply_lite_check` 的评语是编译器风格的诊断文本：先指出错误位置，再引用 prompt 中对应规则的原文。当前检查器会忽略句首语气词（`哈*`、`啊`、`诶`、`哎`、`好家伙`、`呜*`、`前辈`）及其后的 `，！。？`，然后检查回复是否以“这”开头；还会用黑名单拦截 `（拍肩）`、`（递零食）`、`（递奶茶）`、`（递咖啡）`、`（困惑脸）`、`（捂脸）`、`（小声）` 及其半角括号版本。
- 此外还会用高置信模板扫描短括号表达：只看 `（...）` / `(...)` 内 2 到 8 个字、位置接近句尾或独立成分、且不含数字/英文/链接等明显安全特征；只要命中动作/状态词典中的任一词，就直接判违规。
