...
STEP_SYSTEM_INSTRUCTIONS: dict[str, str] = {
    ...
    "vision": (
        "用户是盲人，你需要为他详细描述下面的图片。首先输出图片类型，例如，如果图片是社交平台、网站、游戏、...的截图，类型为`{平台名/游戏名/(无法识别具体平台时留空)}截图`；如果图片包含平台水印，类型为`{平台名}转载`；如果图片为无水印的照片，类型为`照片`；如果图片看起来像表情包，类型为`表情包`；此外，还可标注`艺术作品`、`meme`、......这些类型。。之后，输出图片描述。如果图片中有文字，请添加到结果中。图片描述中不要添加额外解释。\n## 输出示例\n图片类型：微博转载\n图片描述：这是一张照片，内容为[示例中省略]\n"    ),
    "tool": (
